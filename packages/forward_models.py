...
The Newtonian model is a constant-force drift, integrated forward in time when
wall corrections apply. The viscoelastic model is Jeffreys-type creep followed
by recovery after the load is removed at ``t_unload``.
"""
...
class ForwardModels:
    ...
    def _viscoelastic_bounded_full(self, eta_s, eta_p, lambda_, t, Fx, Fy,
        ...
        def ode_eqn(time, state, force_x, force_y):
            ...
            dydt = (force_y / f_perp - Fpy) / solvent_drag
            dFpxdt = (-Fpx + polymer_drag * dxdt) / lambda_
            dFpydt = (-Fpy + polymer_drag * dydt) / lambda_
