...
class Diagnostics:
    ...
    def _gelman_rubin_diag(chain: np.ndarray) -> np.ndarray:
        ...
        return np.sqrt((var_plus + Diagnostics._EPS) / (W + Diagnostics._EPS))
    ...
    _WARMUP_MIN_STEPS = 50    # minimum warm-up steps
    _WARMUP_BALL_SCALE = 0.1  # std of Gaussian ball around best walkers
    ...
    def _warmup_starting_points(self, p0, moves, random_state, progress):
        ...
        scale = np.maximum(self._WARMUP_BALL_SCALE * np.std(chain, axis=0), 1e-8)
